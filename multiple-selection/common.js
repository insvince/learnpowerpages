...
common.functions = {
    //#region 1. Get records
    retrieveEntity: async function retrieveMultipleEntity(url, currentPage) {
        let prefer = `odata.include-annotations=*${currentPage ? `,odata.maxpagesize=${currentPage}` : ''}`;
    ...
    //#region 2. Create a record
    createRecord: function createRecord(record, entityName) {
        return new Promise((resolve, reject) => {
            webapi.safeAjax({
                ...
                success: async function (data, textStatus, xhr) {
                    let newId = await xhr.getResponseHeader('entityid');
                    resolve(newId);
                },
    ...
    //#region 3. Update a record
    updateRecord: function updateRecord(record, entityName, id) {
        return new Promise((resolve, reject) => {
            webapi.safeAjax({
                type: 'PATCH',
                contentType: 'application/json',
                url: `/_api/${entityName}s(${id})`,
                data: JSON.stringify(record),
    ...
    //#region 4. Delete a record
    deleteRecord: function deleteRecord(entitySetName, currentRecordId, callBackFunc, failedCallBack) {
        const requestUrl = `/_api/${entitySetName}(${currentRecordId})`;
        webapi.safeAjax({
            type: 'DELETE',
            url: requestUrl,
            contentType: 'application/json',
